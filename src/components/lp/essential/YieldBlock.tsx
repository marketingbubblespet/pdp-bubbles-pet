      ...
      <div ref={ref} className="max-w-[1100px] mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-center">
          ...
          <div>
            ...
            <h2 className="text-3xl md:text-4xl font-medium leading-tight text-[#0D0C0D] mb-4">
              1 galão. 30 litros. ~300 banhos.
            </h2>
            <p className="text-[#666666] font-medium leading-relaxed mb-8 text-sm md:text-base">
              ...
              {stats.map(({ icon: Icon, value, unit, label }, i) => (
                ...
                >
                  ...
                  <div className="flex items-baseline gap-1">
                    <span className="font-medium text-[#0D0C0D] text-xl md:text-2xl leading-none">{value}</span>
                    <span className="text-[#666666] text-xs">{unit}</span>
                  </div>
                  <span className="text-xs text-[#666666] mt-1">{label}</span>
                </div>
              ))}
            </div>

            <p className="text-[10px] text-[#666666] mb-8">
              * Baseado em 100ml/banho porte pequeno. Varia conforme porte e aplicação.
            </p>

            <CtaLink
              href={PRODUCT.shopifyUrl}
              className="inline-block bg-[#3DB85C] text-white font-semibold text-sm md:text-base px-6 md:px-8 py-3 md:py-4 rounded-[10px] hover:brightness-110 hover:scale-[1.02] active:scale-95 transition-all duration-200 shadow-md"
            >
              ECONOMIZE AGORA →
            </CtaLink>
          </div>
          ...
          >
            ...
            >
              <p className="text-[18px] font-medium text-[#3DB85C] leading-none">R$ 0,73</p>
              <p className="text-[9px] text-[#666666] mt-0.5">por banho</p>
            </div>
          </div>
