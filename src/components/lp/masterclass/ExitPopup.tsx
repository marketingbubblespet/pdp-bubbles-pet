...
export function ExitPopup() {
  ...
  return (
    ...
    >
      <div
        className="relative bg-white rounded-[10px] max-w-[420px] w-full p-6 md:p-8 text-center shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <button
          onClick={() => setOpen(false)}
          aria-label="Fechar"
          className="absolute top-3 right-3 text-[#9ca3af] hover:text-[#0F0C0D] text-2xl leading-none"
        >
          ×
        </button>

        <p className="text-xs font-bold uppercase tracking-widest text-[#E8649A] mb-2">
          Espera!
        </p>
