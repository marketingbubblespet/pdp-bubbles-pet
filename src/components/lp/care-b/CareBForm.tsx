'use client'
// Cópia de src/components/lp/care/CareForm.tsx para a variante B. NENHUM campo, etapa,
// validação ou lógica de submissão foi alterado — só acrescenta `variante: 'b'` nos
// payloads (Netlify/Sellum/dataLayer), pra comparar A x B sem criar evento novo no GTM.
// O card visual foi extraído do wrapper de seção cheia (agora fica dentro do hero da B).
...
export function CareBForm() {
  ...
  const firstInvalidId = (): string | null => {
    if (step === 1) {
      if (form.nome.trim() === '') return 'nome-b'
      if (form.email.trim() !== '' && !isEmailValid(form.email)) return 'email-b'
      if (!isPhoneValid(form.whatsapp)) return 'whatsapp-b'
      return null
    }
    if (step === 2) {
      if (form.categoria === '') return 'grupo-categoria-b'
      if (form.categoria === 'outro' && form.categoriaOutro.trim() === '') return 'categoriaOutro-b'
      if (form.petshop.trim() === '') return 'petshop-b'
      if (form.possuiCnpj === '') return 'grupo-cnpj-b'
      if (form.documento.trim() === '') return 'documento-b'
      if (form.cidade.trim() === '') return 'cidade-b'
      return null
    }
    return null
  }
  ...
  const handleSubmit = async () => {
    ...
    if (sellumResult.status === 'rejected') {
      console.error('Falha ao enviar lead Bubbles Care (variante B) pra Sellum:', sellumResult.reason)
    }
    if (netlifyResult.status === 'rejected') {
      console.error('Falha ao salvar lead Bubbles Care (variante B) no Netlify Forms:', netlifyResult.reason)
    }
    ...
    if (netlifyResult.status === 'fulfilled') {
      pushLeadFromForm({
        ...
        extra: {
          variante: VARIANTE,
          ...(sellumResult.status === 'rejected' ? { sellum_failed: true } : {}),
        },
      })
    }
        ...
        <div className="bg-[#F7F7F7] rounded-2xl p-6 md:p-8 border border-[#E5E7EB]">
          ...
          <div className="flex flex-col gap-4">
            {step === 1 && (
              <>
                <div>
                  <label htmlFor="nome-b" className={labelClass}>Nome completo *</label>
                  <input
                    id="nome-b" type="text" value={form.nome}
                ...
                <div>
                  <label htmlFor="email-b" className={labelClass}>E-mail</label>
                  <input
                    id="email-b" type="email" placeholder="exemplo@gmail.com" value={form.email}
                ...
                <div>
                  <label htmlFor="whatsapp-b" className={labelClass}>WhatsApp *</label>
                  <input
                    id="whatsapp-b" type="tel" inputMode="numeric" placeholder="(00) 00000-0000" value={form.whatsapp}
            ...
            {step === 2 && (
              <>
                <div id="grupo-categoria-b" className="scroll-mt-24">
                ...
                {form.categoria === 'outro' && (
                  <div>
                    <label htmlFor="categoriaOutro-b" className={labelClass}>Qual? *</label>
                    <input
                      id="categoriaOutro-b" type="text" value={form.categoriaOutro}
                ...
                <div>
                  <label htmlFor="petshop-b" className={labelClass}>Nome da empresa *</label>
                  <input
                    id="petshop-b" type="text" value={form.petshop}
                ...
                <div id="grupo-cnpj-b" className="scroll-mt-24">
                  <p className={labelClass}>Possui CNPJ ativo? *</p>
                  <div className="flex gap-3">
                    <RadioPill name="possuiCnpj-b" value="sim" checked={form.possuiCnpj === 'sim'} onChange={() => set('possuiCnpj', 'sim')}>Sim</RadioPill>
                    <RadioPill name="possuiCnpj-b" value="nao" checked={form.possuiCnpj === 'nao'} onChange={() => set('possuiCnpj', 'nao')}>Não</RadioPill>
                  </div>
                ...
                {form.possuiCnpj !== '' && (
                  <div>
                    <label htmlFor="documento-b" className={labelClass}>
                      {form.possuiCnpj === 'sim' ? 'CNPJ *' : 'CPF *'}
                    </label>
                    <input
                      id="documento-b" type="text" inputMode="numeric"
                ...
                <div>
                  <label htmlFor="cidade-b" className={labelClass}>Cidade / Estado *</label>
                  <input
                    id="cidade-b" type="text" value={form.cidade}
            ...
            {step === 3 && (
              <>
                <div>
                  ...
                  <div className="flex gap-3">
                    <RadioPill name="clienteBubbles-b" value="sim" checked={form.clienteBubbles === 'sim'} onChange={() => set('clienteBubbles', 'sim')}>Sim</RadioPill>
                    <RadioPill name="clienteBubbles-b" value="nao" checked={form.clienteBubbles === 'nao'} onChange={() => set('clienteBubbles', 'nao')}>Não</RadioPill>
                  </div>
                </div>

                <div id="grupo-investimento-b" className="scroll-mt-24">
                  <label htmlFor="investimento-b" className={labelClass}>
